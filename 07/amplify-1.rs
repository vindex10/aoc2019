...
fn run(program:&Vec<i32>, inputs:&Vec<i32>) -> Vec<i32> {
    let max = program.len();
    let mut line: Vec<i32> = (*program).iter().map(|s| *s).collect();
    let mut i = 0;
    let mut cmd;
    let mut args;
    let mut to;
    ...
    while i < max {
        ...
        if cmd == 1 {
            ...
            println!("{}, {}", cmd, to);
            line[to] = *input_iter.next().unwrap();
            println!("{}: {}", to, line[to]);
            i += 2;
        } else if cmd == 4 {
...
fn main() -> Result<()> {
    ...
    let perms = (0..5 as i32).permutations(5);
    let mut res = -1;
    let mut best;
    for perm in perms {
        let mut out = 0;
